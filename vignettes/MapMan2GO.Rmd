...
The following R script does the job, saves the resulting data.frames in binary format in this R package's data folder and generates some plots about the assignment's quality and trustworthiness. 

`Rscript ./exec/mapBinsToGOs.R  path/2/MercatorRawResults.txt path/2/MapMan2GO`

Aditionally, a script is provided to generate the compound Gene Ontoly Annotation for a list of selected protein IDs.
...
Rscript ./exec/evalGoPredictionPerformances_NonSwissprot.R inst/nonSwissProtGenesWithNonElectMadeGOAs_trEMBL_mercator_result.tsf inst/nonSwissProtGenesWithNonElectMadeGOAs_trEMBL_interProScan_result.tsv ../goa_uniprot_all_tbl.txt ../performance_evaluation/gene_association.gramene_oryza path/2/MapMan2GO
```
...
```
Generates histograms for precision, recall, false positive rate, F1-Score and Matthews correlation coefficient, as well as scatter plots of false positives, true negatives and number of predictions for Mercator and InterProScan performances. 
...
```
Generate a text file containing the performance evaluation of the two methods, including and excluidng the ancestral GO terms. This is done for all the proteins as well as for the three sub-ontologies.
