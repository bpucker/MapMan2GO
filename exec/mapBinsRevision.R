...
for (i in 1:length(mm.2.go.df$MapManBin)) {
    ...
    li.used <- table(unlist(mclapply(eco.used.gos, function(x) {
        evidenceCodeBins(x)
    })))
    ...
    li.no.used <- table(unlist(mclapply(eco.no.used.gos, function(x) {
        evidenceCodeBins(x)
    })))
...
for (i in 1:length(duplicated.goas)) {
    ...
    for (j in 1:length(bins.1.goa)) {
        a <- paste("mm.2.go$`", paste(bins.1.goa[j], "`$MapManBin.GO", sep = ""), 
            sep = "")
        o.go <- sort(unique(unlist(eval(parse(text = a)))))
...
boxplot(trusted.used, untrusted.used, trusted.no.used, untrusted.no.used, col = c("lightgrey", 
    "lightgrey", "white", "white"), main = "Trustworthiness of GO Terms Annotations", 
    pch = "-", names = c("Trusted", "Untrusted", "Trusted", "Untrusted"))
legend("topright", inset = 0.05, c("Used", "No used"), fill = c("lightgrey", "white"))
dev.off()

save(sh.entropy, means.bins.rel.ref.abund, trusted.used, untrusted.used, trusted.no.used, 
    untrusted.no.used, file = file.path(input.args[[1]], "data", "MapManBinsRevision.RData"))
